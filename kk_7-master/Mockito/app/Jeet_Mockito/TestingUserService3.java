...
public class TestingUserService3 {
	@TestConfiguration
...
List<User> checker1 = new ArrayList<User>();
List<User> checker2 = new ArrayList<User>();
checker1.add(two);
checker1.add(four);
checker1.add(one);
checker1.add(three);
...
if(us.getUsersSorted("phone", db.findAll()).equals(checker1)) {
	isRight = true;
}
else {
	isRight = false;
}
if(us.getUsersSorted("phone", db.findAll())!=checker2) {
	isWrong = true;
}
else {
	isWrong = false;
